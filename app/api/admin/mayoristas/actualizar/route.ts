...
export async function POST(request:Request){
  ...
  const datos=await request.json();const {data:anterior}=await admin.from('clientes_mayoristas').select('email,contacto_nombre,estado').eq('id',datos.id).eq('empresa_id',perfil.empresa_id).single();
