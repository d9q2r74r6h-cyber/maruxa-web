...
function trabajaEnSuDescanso(linea: Linea, fecha: string, funcionarios: Funcionario[]) { const persona=funcionarios.find((item)=>item.id===linea.funcionario_id); if(!persona?.dia_descanso)return false; const dia=DIAS_SEMANA[new Date(`${fecha}T12:00:00`).getDay()]; return dia===textoComparable(persona.dia_descanso); }
function lineaParaPago(linea: Linea, fecha: string, funcionarios: Funcionario[]) { return trabajaEnSuDescanso(linea,fecha,funcionarios)?{...linea,origen:'externo' as Origen}:linea; }
function lineasCalculadas(turno: Turno, esFestivo: boolean, config: ConfigPago = CONFIG_PAGO_BASE, fecha = hoy, funcionarios: Funcionario[] = []) {
  ...
  return turno.lineas.map((item) => {
    const itemPago=lineaParaPago(item,fecha,funcionarios);
    const demasiaIndividual = recibeDemasia(item) ? demasia : 0;
    const monto = item.nombre.trim() ? montoBase(itemPago, esFestivo, demasiaIndividual, config) : 0;
    const dominical = correspondeDominical(itemPago, fecha, funcionarios) ? monto : 0;
    return { ...item, origen:itemPago.origen, monto, monto_es_base: true, dominical, total_pago: monto + demasiaIndividual + totalBonos(item) + dominical };
  });
}
...
function imprimirHojaTurno(titulo: string, fecha: string, qq: number, lineas: Linea[], esFestivo: boolean, configPago: ConfigPago, funcionarios: Funcionario[], posicionTurno = 0) {
  ...
  ventana.document.head.insertAdjacentHTML('beforeend','<style>h1,.declaracion,.pie{display:none!important}header{padding-bottom:5px!important;margin-bottom:6px!important}.datos{font-size:10px!important}table{font-size:9px!important}th{padding:4px 3px!important}td{height:34px!important;padding:4px 3px!important}.monto{font-size:11px!important}</style>');
  if(posicionTurno%2===1){
    ventana.document.body.style.paddingTop='98mm';
    ventana.document.head.insertAdjacentHTML('beforeend','<style>header{padding-bottom:5px!important;margin-bottom:6px!important}h1{font-size:18px!important;margin-bottom:4px!important}.datos{font-size:10px!important}table{font-size:9px!important}th{padding:4px 3px!important}td{height:34px!important;padding:4px 3px!important}.monto{font-size:11px!important}.declaracion{margin-top:6px!important;font-size:9px!important}.pie{margin-top:14px!important}.linea{padding-top:4px!important;font-size:8px!important}</style>');
  }
...
function EditorLineas({ titulo, fecha, lineas, onChange, onRemove, panaderos, posicionTurno = 0, qq = 0, esFestivo = false, onQq, configPago = CONFIG_PAGO_BASE, funcionariosPanaderos = [], conceptosBono = [] }: { titulo: string; fecha?: string; lineas: Linea[]; onChange: (lineas: Linea[]) => void; onRemove?: () => void; panaderos?: boolean; posicionTurno?: number; qq?: number; esFestivo?: boolean; onQq?: (valor: number) => void; configPago?: ConfigPago; funcionariosPanaderos?: Funcionario[]; conceptosBono?: ConceptoBono[] }) {
  ...
  const calculadas = panaderos ? lineasCalculadas({id:'',nombre:titulo,qq,lineas},esFestivo,configPago,fecha||hoy,funcionariosPanaderos) : lineas;
  const dominicalTotal = calculadas.reduce((suma,linea)=>suma+numero(linea.dominical),0);
  const total = calculadas.reduce((suma,linea)=>suma+(panaderos&&!linea.nombre.trim()?0:panaderos?numero(linea.total_pago):numero(linea.monto)),0);
  return (
    <section className="rounded-xl border border-[#4B2818]/15 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">{panaderos && <><label className="flex items-center gap-2 text-xs font-black">QQ<input value={qqTexto} inputMode="decimal" onChange={(event) => { if (/^\d*(?:[.,]\d*)?$/.test(event.target.value)) setQqTexto(event.target.value); }} onBlur={() => onQq?.(Math.max(0, numeroDecimal(qqTexto)))} onKeyDown={(event) => { if (event.key === 'Enter') event.currentTarget.blur(); }} placeholder="Ej: 1,5" className="h-8 w-20 rounded border px-2 text-right" /></label><button type="button" onClick={()=>imprimirHojaTurno(titulo,fecha||hoy,qq,lineas,esFestivo,configPago,funcionariosPanaderos,posicionTurno)} className="inline-flex h-8 items-center gap-1 rounded-md border border-[#A51F2B]/30 px-2 text-[10px] font-black uppercase text-[#A51F2B]" title={posicionTurno%2===1?'Imprime este turno desde la mitad inferior de la hoja':'Imprime este turno en la mitad superior de la hoja'}><Printer className="h-3.5 w-3.5" />Firmas</button></>}{!panaderos && <span className="rounded-full bg-[#FFF3DF] px-3 py-1 text-sm font-black text-[#A51F2B]">{dinero(total)}</span>}{onRemove && <button type="button" onClick={onRemove} className="grid h-8 w-8 place-items-center rounded-md text-red-700 hover:bg-red-50" aria-label={`Eliminar ${titulo}`}><Trash2 className="h-4 w-4" /></button>}</div>
      </div>
      <div className="mt-3 space-y-2">
        ...
        {lineas.map((linea, indice) => (
          <div key={linea.id} className={`grid gap-1.5 ${panaderos ? 'grid-cols-[minmax(110px,1fr)_68px_78px_82px_88px_92px_26px]' : 'grid-cols-[minmax(0,1fr)_130px_36px]'}`}>
            ...
            {panaderos && <><select value={trabajaEnSuDescanso(linea,fecha||hoy,funcionariosPanaderos)?'externo':linea.origen||'casa'} disabled={trabajaEnSuDescanso(linea,fecha||hoy,funcionariosPanaderos)} title={trabajaEnSuDescanso(linea,fecha||hoy,funcionariosPanaderos)?'Externo automatico: trabaja en su dia de descanso':'Origen del pago'} onChange={(event) => onChange(lineas.map((item, posicion) => posicion === indice ? { ...item, origen: event.target.value as Origen, monto_manual: item.tipo_eventual === 'aprendiz' } : item))} className="h-10 rounded-md border bg-white px-2 text-xs font-black disabled:border-amber-400 disabled:bg-amber-50 disabled:text-amber-900"><option value="casa">Casa</option><option value="externo">Externo</option></select><select value={linea.funcion || 'oficial'} onChange={(event) => onChange(lineas.map((item, posicion) => posicion === indice ? { ...item, funcion: event.target.value as Funcion, monto_manual: item.tipo_eventual === 'aprendiz' } : item))} className="h-10 rounded-md border bg-white px-2 text-xs font-black"><option value="batea">Batea</option><option value="cocedor">Cocedor</option><option value="oficial">Oficial</option></select></>}
            ...
            {panaderos ? <><SueldoInput valor={linea.nombre.trim()?numero(calculadas[indice]?.monto):0} nombre={linea.nombre} manual={linea.monto_manual} onCommit={(monto) => onChange(lineas.map((item, posicion) => posicion === indice ? { ...item, monto, monto_manual: true, monto_es_base: true } : item))} /><div title="Total del panadero: base + demasia + bonos + dominical" className="grid h-10 place-items-center rounded-md bg-[#2A1710] px-1 text-right text-xs font-black text-white">{linea.nombre.trim()?dinero(numero(calculadas[indice]?.total_pago)):dinero(0)}</div></> : <input type="text" inputMode="numeric" value={linea.monto || ''} onChange={(event) => onChange(lineas.map((item, posicion) => posicion === indice ? { ...item, monto: Math.max(0, numero(event.target.value)) } : item))} placeholder="$0" className="h-10 rounded-md border border-[#4B2818]/20 px-3 text-right text-sm font-black" />}
      ...
      <fieldset disabled={bloqueada} className="space-y-5 disabled:opacity-75">
        <div className="grid gap-5 xl:grid-cols-2">
          <div className="space-y-5">
            {turnos.map((turno,indiceTurno) => <EditorLineas key={turno.id} titulo={`Panaderos · ${turno.nombre}`} fecha={fecha} lineas={turno.lineas} panaderos posicionTurno={indiceTurno} qq={turno.qq || 0} esFestivo={esFestivo} configPago={configPago} funcionariosPanaderos={funcionariosPanaderos} conceptosBono={conceptosBono} onQq={(qq) => setTurnos((actuales) => actuales.map((item) => item.id === turno.id ? { ...item, qq } : item))} onChange={(lineas) => setTurnos((actuales) => actuales.map((item) => item.id === turno.id ? { ...item, lineas } : item))} onRemove={turnos.length > 1 ? () => setTurnos((actuales) => actuales.filter((item) => item.id !== turno.id)) : undefined} />)}
